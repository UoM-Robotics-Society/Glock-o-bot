...
const int Tune3BeatDenom = 8;
  ...
  // Test Notes
const unsigned int Tune1[Tune1NoteMax][Tune1Length] PROGMEM = {{79,80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,96,97,98,99,100,101,102,103,104,105,106,107,108}};
  // Harry Potter
const unsigned int Tune2[Tune2NoteMax][Tune2Length] PROGMEM = {{83,0,88,0,0,91,90,0,88,0,0,0,95,0,93,0,0,0,0,0,90,0,0,0,0,0,88,0,0,91,90,0,86,0,0,0,89,0,83,0,0,0,0,0,0,0,0,0,83,0,88,0,0,91,90,0,88,0,0,0,95,0,98,0,0,0,97,0,96,0,0,0,92,0,96,0,0,95,94,0,90,0,0,0,91,0,88,0,0,0,0,0,0,0,0,0,91,0,95,0,0,0,91,0,95,0,0,0,91,0,96,0,0,0,95,0,94,0,0,0,90,0,91,0,0,95,94,0,82,0,0,0,83,0,95,0,0,0,0,0,0,0,0,0,91,0,95,0,0,0,91,0,95,0,0,0,91,0,98,0,0,0,97,0,96,0,0,0,92,0,96,0,0,95,94,0,90,0,0,0,91,0,88}};
  // Addams Family
const unsigned int Tune3[Tune3NoteMax][Tune3Length] PROGMEM = {
    ...
    {    
          ...
          for (BeatCount=0; BeatCount < Tune_Info[SongCount][0]; BeatCount++)
          {
                ...
                { 
                      ...
                      {
                          case 0:
                            Load_Note = pgm_read_byte(&Tune1[NoteCount][BeatCount]);
                            break;
                          case 1:
                            Load_Note = pgm_read_byte(&Tune2[NoteCount][BeatCount]);
                            break;
                          case 2:
                            Load_Note = pgm_read_byte(&Tune3[NoteCount][BeatCount]);
                            break;
                      }
...
void to_BCD(int number)
{
    if (number == 0) //write 0000
    {
        digitalWrite(A, LOW);
        digitalWrite(B, LOW);
        digitalWrite(C, LOW);
