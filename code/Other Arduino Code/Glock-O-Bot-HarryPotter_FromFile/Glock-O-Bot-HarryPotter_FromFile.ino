...
String SettingsFile = "Harry Potter Settings.txt"; // File containing the settings, [0]-Number of notes, [1]-Tempo(bpm)
String MusicFile = "Harry Potter Music.txt";          // Notes to be played
...
int Beat_Denom;     // Smallest denomination of a beat to be played, i.e. 1 for whole beat, 4 for ...
const int Note_Max = 1; // Max number of notes played at the same time
int Load_Note;
...
{
    // Load Settings File
    LoadFile = SD.open(SettingsFile,FILE_READ);  
    if (LoadFile) 
    {
    ...
    NumNotes = Settings[0];
    Tempo = Settings[1];
    Beat_Denom = Settings[2];
    
    Beat_Time = (1/(Tempo/60))/Beat_Denom*1000000;
    Serial.print("Beat Time (s): ");
    Serial.println(Beat_Time/1000000);

    
    // Load Music File
    int Notes[NumNotes];  // Create temp array for loading the notes to be played
    LoadFile = SD.open(MusicFile,FILE_READ);  
    if (LoadFile) 
    {
    ...
    //int Notes[NumNotes] PROGMEM = {TempNotes};
    ...
    while (On_Signal > 0)
    {
        On_Signal = digitalRead(Graphene_Sen_P); //  Wait for Graphene  ON (LOW)
    }
    On_Signal = 1; // debounce 1msec
    ...
    for (int Beat_Count=0; Beat_Count < NumNotes; Beat_Count++)
    {
          
        for (int Note_Count=0; Note_Count < Note_Max; Note_Count++)
        {
                
                Load_Note = (Notes[Beat_Count]);
                if (Load_Note >= 149)
                {
                     // Volume 5
    ...
    while (On_Signal < 1)
    {
        On_Signal = digitalRead(Graphene_Sen_P); //  Wait for Graphene signal OFF (HIGH)
    }
...
//-----------------
    ...
    part_tempo = ((NumNotes/Beat_Denom)/part_time)*60;    
