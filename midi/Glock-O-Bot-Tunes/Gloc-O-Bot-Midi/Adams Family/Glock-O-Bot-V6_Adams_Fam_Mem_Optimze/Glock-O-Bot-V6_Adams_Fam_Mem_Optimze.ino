...
// TUNE B DEFINED (Derrived from MIDI file)

#define Tune_B_Length 59 // Actual tune length = 59
#define Note_B_Max 1 // Max number of notes struck per beat = 4
const long Beat_B_Time = 125000; // Note time in uSec

//Enter the tune here (79-108) anything above this range or below this range has been scaled to reprisent volume

const unsigned char Tune_B[Note_B_Max][Tune_B_Length]PROGMEM = {79,80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,96,97,98,99,100,101,102,103,104,105,106,107,108,107,106,105,104,103,102,100,99,98,97,96,95,94,93,92,91,90,89,88,87,86,85,84,83,82,81,80,79};
//const unsigned char Tune_B[Note_B_Max][Tune_B_Length]PROGMEM = {83,0,88,0,0,91,90,0,88,0,0,0,95,0,93,0,0,0,0,0,90,0,0,0,0,0,88,0,0,91,90,0,86,0,0,0,89,0,83,0,0,0,0,0,0,0,0,0,83,0,88,0,0,91,90,0,88,0,0,0,95,0,98,0,0,0,97,0,96,0,0,0,92,0,96,0,0,95,94,0,90,0,0,0,91,0,88,0,0,0,0,0,0,0,0,0,91,0,95,0,0,0,91,0,95,0,0,0,91,0,96,0,0,0,95,0,94,0,0,0,90,0,91,0,0,95,94,0,82,0,0,0,83,0,95,0,0,0,0,0,0,0,0,0,91,0,95,0,0,0,91,0,95,0,0,0,91,0,98,0,0,0,97,0,96,0,0,0,92,0,96,0,0,95,94,0,90,0,0,0,91,0,88};
                                              
// TUNE C DEFINED (Derrived from MIDI file)

#define Tune_C_Length 183 // Actual tune length = 183
#define Note_C_Max 1 // Max number of notes struck per beat = 4
const long Beat_C_Time = 125000; // Note time in uSec

//Enter the tune here (79-108) anything above this range or below this range has been scaled to reprisent volume

const unsigned char Tune_C[Note_C_Max][Tune_C_Length]PROGMEM = {83,0,88,0,0,91,90,0,88,0,0,0,95,0,93,0,0,0,0,0,90,0,0,0,0,0,88,0,0,91,90,0,86,0,0,0,89,0,83,0,0,0,0,0,0,0,0,0,83,0,88,0,0,91,90,0,88,0,0,0,95,0,98,0,0,0,97,0,96,0,0,0,92,0,96,0,0,95,94,0,90,0,0,0,91,0,88,0,0,0,0,0,0,0,0,0,91,0,95,0,0,0,91,0,95,0,0,0,91,0,96,0,0,0,95,0,94,0,0,0,90,0,91,0,0,95,94,0,82,0,0,0,83,0,95,0,0,0,0,0,0,0,0,0,91,0,95,0,0,0,91,0,95,0,0,0,91,0,98,0,0,0,97,0,96,0,0,0,92,0,96,0,0,95,94,0,90,0,0,0,91,0,88};
...
void loop() 
  ...
  Graphene_Start(); //Wait for Graphene Start Signal
  
  Next_Beat = 0; // Reset Flag ADDED keep here as interupt can change it 
  Beat_Time = Beat_A_Time; //Set tempo for Tune A
  Start_Beat_Timer(); // Start Timmer 1 with correct Tempo
      
  for (int Beat_Count=0; Beat_Count < Tune_A_Length; Beat_Count++)
      {
        
       for (int Note_Count=0; Note_Count < Note_A_Max; Note_Count++)
            {
              Load_Note = pgm_read_byte(&Tune_A[Note_Count][Beat_Count]);
              if (Load_Note >= 149)
                {
                     // Volume 5
  ...
  Graphene_Cleared(); // Wait for Graphene signal to reset
   
// End of tune A, start of Tune B
  ...
  Next_Beat = 0; // Reset Flag ADDED keep here as interupt can change it 
  Beat_Time = Beat_B_Time; //Set tempo for Tune B
  Start_Beat_Timer(); // Start Timmer 1 with correct Tempo
      
  for (int Beat_Count=0; Beat_Count < Tune_B_Length; Beat_Count++)
      {
        
       for (int Note_Count=0; Note_Count < Note_B_Max; Note_Count++)
            {
              Load_Note = pgm_read_byte(&Tune_B[Note_Count][Beat_Count]);
              if (Load_Note >= 149)
                {
                     // Volume 5
                     Solenoid_On_Time_2 = 1300;
  ...
  Next_Beat = 0; // Reset Flag ADDED keep here as interupt can change it 
  Beat_Time = Beat_C_Time; //Set tempo for Tune A
  Start_Beat_Timer(); // Start Timmer 1 with correct Tempo
      
  for (int Beat_Count=0; Beat_Count < Tune_C_Length; Beat_Count++)
      {
        
       for (int Note_Count=0; Note_Count < Note_C_Max; Note_Count++)
            {
              Load_Note = pgm_read_byte(&Tune_C[Note_Count][Beat_Count]);
              if (Load_Note >= 149)
                {
                     // Volume 5
                     Solenoid_On_Time_2 = 1300;
